...
struct Input {
    ...
    event_data: u32, // The id we use to trigger the event
}
fn main() {
    ...
    let input_throttle_set = Input {
        event: "THROTTLE_SET".to_string(),
        input_id: 4,
        event_data: 0
    };
    ...
    events.insert(input_throttle_set.input_id, input_throttle_set);
            ...
            Some(event) => {
                ...
                */
                conn.transmit_client_event(
                    0,
                    input_id as u32,
                    events.get(&input_id).unwrap().event_data,
